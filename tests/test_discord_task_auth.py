...
def test_authorize_rejects_cross_guild_access(action: DiscordTaskAction) -> None:
    with pytest.raises(DiscordTaskAuthorizationError, match="guild"):
        authorize(_record(), action, _access(guild_id=99))


@pytest.mark.parametrize("revoked_channel", (3, 4))
@pytest.mark.parametrize("action", list(DiscordTaskAction))
def test_authorize_rejects_revoked_channel_visibility(
    action: DiscordTaskAction, revoked_channel: int
) -> None:
...
@pytest.mark.parametrize("action", list(DiscordTaskAction))
def test_owner_can_perform_every_action(action: DiscordTaskAction) -> None:
