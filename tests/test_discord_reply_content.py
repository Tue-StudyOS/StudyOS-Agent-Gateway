...
def test_short_reply_stays_inline(tmp_path: Path) -> None:
    prepared = prepare_discord_reply("Yep, that race is fixed.", (), tmp_path, "task-123")
...
def test_fenced_code_is_moved_to_markdown_attachment(tmp_path: Path) -> None:
    ...
    prepared = prepare_discord_reply(message, (), tmp_path, "task-123")
    ...
    assert prepared.files[0].name == "reply-task-123.md"
...
def test_long_reply_keeps_existing_artifacts_and_adds_markdown(tmp_path: Path) -> None:
    ...
    prepared = prepare_discord_reply(message, (image,), tmp_path, "task-456")

    assert prepared.files == (image, tmp_path / "discord-replies/reply-task-456.md")
...
def test_long_reply_fails_before_exceeding_discord_attachment_limit(tmp_path: Path) -> None:
    ...
    with pytest.raises(RuntimeError, match="attachment limit"):
        prepare_discord_reply("detail " * 200, files, tmp_path, "task-789")

    assert not (tmp_path / "discord-replies/reply-task-789.md").exists()


def test_reply_attachment_uses_task_delivery_key(tmp_path: Path) -> None:
    delivery_key = "a" * 32

    prepared = prepare_discord_reply("detail " * 200, (), tmp_path, delivery_key)
